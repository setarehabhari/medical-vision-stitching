# Medvit model file forked from MEDVIT github repository
"""
Author: Omid Nejati
...
class E_MHSA(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        q = self.q(x)
        q = q.reshape(B, N, self.num_heads, int(C // self.num_heads)).permute(0, 2, 1, 3).to(torch.float64)
        ...
        if self.sr_ratio > 1:
            ...
            x_ = x_.transpose(1, 2)
            k = self.k(x_)
            k = k.reshape(B, -1, self.num_heads, int(C // self.num_heads)).permute(0, 2, 3, 1).to(torch.float64)
            v = self.v(x_)
            ...
            v = v.reshape(B, -1, self.num_heads, int(C // self.num_heads)).permute(0, 2, 1, 3)
        attn = (q @ k) * self.scale
        attn = attn.softmax(dim=-1).to(torch.float)
...
class MedViT(nn.Module):
    def __init__(self, stem_chs, depths, path_dropout, attn_drop=0, drop=0, num_classes=1000,
        ...
        for stage_id in range(len(depths)):
            ...
            idx += numrepeat
            self.stage_end_id.append(len(features))
    ...
    def extract_block_features(self, x):
        ...
        size_dict = {}
        start_idx = -1
        for idx, end_idx in enumerate(self.stage_end_id):
            stage_feats = {}
            stage_size = {}
            stage_feats[-1] = x.detach()
            stage_size[-1] = stage_feats[-1].size()
            for i, blk in enumerate(self.features[start_idx+1:end_idx]):
                x = blk(x)
                stage_feats[i] = x.detach()
                stage_size[i] = stage_feats[i].size()
            block_features[idx] = stage_feats
            size_dict[idx] = stage_size
            start_idx = end_idx
            x = self.features[end_idx:end_idx+1](x)
        print("size_dict extract")
        print(size_dict)
    ...
    def forward_until(self, x, stage_id, blk_id):
        x = self.stem(x)
        stage_start_id = [0] + self.stage_end_id[:-1]
        idx = stage_start_id[stage_id] + (blk_id + 1)
        for blk in self.features[:idx]:
            x = blk(x)
        return x

    def forward_from(self, x, stage_id, blk_id):
        stage_start_id = [0] + self.stage_end_id[:-1]
        idx = stage_start_id[stage_id] + blk_id
        for blk in self.features[idx:]:
            x = blk(x)
...
@register_model
def MedViT_nano(pretrained=False, pretrained_cfg=None, **kwargs):
    model = MedViT(stem_chs=[64, 32, 64], depths=[2, 3, 5, 2], path_dropout=0.02, **kwargs)
    return model


@register_model
def MedViT_micro(pretrained=False, pretrained_cfg=None, **kwargs):
    model = MedViT(stem_chs=[64, 32, 64], depths=[3, 4, 5, 3], path_dropout=0.05, **kwargs)
    return model
