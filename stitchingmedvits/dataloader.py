...
class NEHOCTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = Image.open(img_name) #pydicom.dcmread(img_name).pixel_array
...
class NEHOCTNoisyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            image = self.transform(image)
            if p > 0.5:
                mean = 0.589 #noprespective0.630 #prespective0.572 #justneh 0.589
                std = -0.413 #noprespective00.382 #0.406 #-0.413
            else:
                mean = 0.099 #0.138 #0.123 #0.099
                std = 0.207 #0.211 #0.208 #0.207
